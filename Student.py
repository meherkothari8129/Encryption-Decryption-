...
firebase = firebase.FirebaseApplication("https://python-a93bd-default-rtdb.firebaseio.com", None)
login_window = Tk()
...
def getData():
    ...
    get_friend_data = firebase.get('/', your_friends_code)
    if(get_friend_data != None):
        print("data : ", get_friend_data)
        byte_str = bytes.fromhex(get_friend_data)
        original = decrypt('AIM', byte_str)
        final_message = original.decode("utf-8")
